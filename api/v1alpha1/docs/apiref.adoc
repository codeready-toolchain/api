...
| *`phoneLookupMode`* __string__ | PhoneLookupMode controls how the registration service handles Twilio Lookup v2 phone risk checks. +
Valid values are "disabled" (skip Lookup entirely), "log" (call Lookup and store results but don't block), +
and "enabled" (call Lookup and enforce blocking). Defaults to "log". + |  | Enum: [disabled log enabled] +
Optional: \{} +
