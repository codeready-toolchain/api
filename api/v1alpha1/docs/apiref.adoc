...
| *`parameters`* __xref:{anchor_prefix}-github-com-codeready-toolchain-api-api-v1alpha1-parameter[$$Parameter$$] array__ | Parameters is an optional array of Parameters used during the NSTemplateTier and TierTemplate creation. +
...
| *`revisions`* __object (keys:string, values:string)__ | Revisions is an array containing the names of the last applied TierTemplateRevision CRs. +
