...
|===
| Field | Description | Default | Validation
| *`memoryUsagePercentPerNode`* __object (keys:string, values:integer)__ | MemoryUsagePercentPerNodeRole is the percent of the memory used per node role (eg. worker, master) + |  | 
